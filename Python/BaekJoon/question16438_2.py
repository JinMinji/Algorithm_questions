...
def dp_made(n):
    ...
    if n % 2 == 0:  # 짝수
        ...
        h = n // 2
        tmp_list1 = copy.deepcopy(dp(h))
        tmp_list2 = copy.deepcopy(dp(h + 1))
        ...
        if len(tmp_list1) != len(tmp_list2):
            tmp_list = copy.deepcopy(dp(h))
            tmp_list[i].extend(tmp_list2[i + 1])
            result.append(tmp_list[i])
